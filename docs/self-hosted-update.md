...
## 前提: Chromeブラウザのクラウド管理登録が必要

自己ホストの拡張機能を強制インストールするには、**そのPCが「企業の管理対象」として
検出されている必要がある**（Chromeの仕様）。ローカルにポリシーを書くだけでは、
以下の警告が出て拒否される。

> このパソコンは企業の管理対象として検出されていないため、ポリシーで自動的に
> インストールできるのは Chrome ウェブストアでホストされている拡張機能のみとなります。

このため、先にGoogle管理コンソールでの登録が必要になる。
管理者への依頼内容は `docs/admin-request.md` を参照。

### 登録トークンを受け取ったあとの手順（各メンバーのMacで1回ずつ）
...
Chromeを完全に終了（⌘Q）して再起動する。

`chrome://policy` を開き、上部に「このブラウザは <組織名> によって管理されています」
と表示されれば登録成功。管理コンソールの「管理対象ブラウザ」一覧にも表示される。

登録後は、拡張機能の配布設定は管理コンソール側（クラウドポリシー）で行う。
ローカルの `ExtensionInstallForcelist` は不要なので設定しない。
