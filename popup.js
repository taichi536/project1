...
以下の候補者プロフィール${positionName ? 'と応募ポジション情報' : ''}を読んで、スカウトメールに挿入するパーソナライズ文を1文で作成してください。
${positionSection}
【挿入位置】
...
【良い例】
・「大手SIerでのERPシステム導入においてPMとして顧客折衝から要件定義・リリースまで一気通貫で担われてきた点が印象的でした。」
・「証券リテール営業10年で個人成果と支店マネジメントの両面を担われ、育成体制の構築まで手掛けてこられた経緯が拝見でき、大変魅力的なご経歴と感じた次第です。」
...
- 年収がプロフィールに明記されており、以下の閾値を下回る → 必ずNG（情報が明確な場合は「迷う」余地なし）。年収が幅（例：750〜1000万円）で記載されている場合は上限値で判断し、上限値が基準以上であればNGにしない。
  ・20代: ${v('age20s', 400)}万円未満
  ・30〜35歳: ${v('age30to35', 550)}万円未満
