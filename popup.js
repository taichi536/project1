...
// スカウト文のパーソナライズ段落だけ、他の処理より上位のモデルを使う。
//
// ここは候補者が実際に読む唯一の文で、文章の自然さがそのまま返信率に効く。
// 他の処理（スクリーニング・属性抽出）は判定なので Haiku で足りるが、
// 文章を書かせる用途では語尾や言い回しが型にはまりやすい。
// 出力は300トークンまでなので、上位モデルにしても1通あたりの増加はわずか。
const SCOUT_LINE_MODEL = 'claude-sonnet-5';
...
` : '';
  ...
  const prompt = `ハイクラス転職エージェントの代表が候補者に送るスカウトメールの、一段落を書いてください。

【入る場所】
直前の段落：「この度、貴方様のご経歴を拝見し、${splitPositionLabel(positionName).company ? splitPositionLabel(positionName).company + 'の' : ''}「${splitPositionLabel(positionName).position || '〇〇'}」のポジションに高い親和性を感じ、ご連絡いたしました。」
直後の段落：「当方の経験上、面接次第ではありますが、かなり高い確度で本ポジションにてオファーが出ると感じます。」

前後がどちらも「〜感じ」「〜感じます」で終わっています。ここで同じ結び方をすると、
3段落続けて同じ語尾になります。別の終わり方にしてください。

【書くこと】
その人の経歴を読んで具体的に目を引いた点。数値で語れる実績（予算達成率・前年比・
マネジメント人数・取扱件数・受賞・登壇など）があれば必ず挙げる。数値が無い場合は、
担当した領域や役割の具体性で書く。無い数字を作らない。

【長さ】
1文か2文。1文に詰め込んで修飾が重なるより、短く区切ってよい。

【書き方】
実際に経歴を読んだ人が書いた文章にする。下の対比を参考にすること。

✕ 証券リテール営業として個人予算を3年連続で120%超達成されつつ、支店マネジメントとして5名の育成も担われてきた点が印象的でした。
◯ 個人予算を3年連続で120%超えられながら、同時に5名の育成も見ておられたのですね。営業と育成を両方回せる方は、そう多くありません。

✕ 金融機関での現場営業を土台に、提携銀行への販売推進では研修設計から同行サポートまで構造的に支援を組み立ててこられており、このポジションをはじめ幅広い可能性を感じております。
◯ 提携銀行への販売推進で、研修の設計から同行まで一通り手がけられています。売る側と教える側の両方が分かる方を、この求人は探しています。

✕ 大手SIerにてPMとして大規模プロジェクトを牽引され、豊富なご経験をお持ちでいらっしゃいます。
◯ 100名規模のプロジェクトでPMを務められたご経験は、このポジションが求めているものとほぼ重なります。

【避けること】
- 「〜視座」「〜発想転換」「〜思考プロセス」「構造的に」のような硬い分析語
- 「即戦力」「希少」などの評価語
- 「まさに〜そのもの」「〜こそが〜」のような大げさな断言
- 「〜点が印象的でした」「〜でいらっしゃいます」
- 「高い親和性」の繰り返し（直前の段落で既に使っています）
- 「ぜひ〜」「お話しさせていただきたく」
- 「〇〇様」など名前から書き始めること

【出力】
本文だけを書く。前置き・説明・補足・注記は書かない。

【候補者プロフィール】
${profileText}`;

  const data = await claudeFetch(apiKey, {
    model: SCOUT_LINE_MODEL,
    max_tokens: 300,
