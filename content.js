...
function buildCriteriaText(criteria, platform) {
  ...
  if (isBizreach) {
    ...
    lines.push(`- 学歴：以下のいずれかに該当する場合のみOK。それ以外はNG。`);
    lines.push(`  ① 早慶上智（早稲田・慶應・上智）→ 無条件OK`);
    lines.push(`  ② MARCH（明治・青山学院・立教・中央・法政）→ 無条件OK`);
    lines.push(`  ③ 国立大学（旧帝大・東工大・一橋・筑波・横浜国立・小樽商科・北海道・東北・名古屋・大阪・九州・神戸・広島・岡山・千葉・埼玉・金沢・信州・静岡・熊本・長崎・鹿児島・琉球等、国が設置する全ての大学）→ 無条件OK`);
    lines.push(`  ④ 関関同立（関西大・関西学院大・同志社大・立命館大）かつ大手企業・外資系・有名上場企業に現在在籍中 → OK`);
    lines.push(`  ※学歴が不明・記載なしの場合は学歴NGとせず、他の条件で判断すること（学歴不明→要確認ではなく、学歴以外の条件を適用）`);
