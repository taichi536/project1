...
function buildCriteriaText(criteria, platform) {
  ...
  lines.push(`- 「ITエンジニア系（IT・理系・技術職すべて）」: ソフトウェアエンジニア・SE・インフラ・クラウド・システム構築・データエンジニア・データサイエンティスト・データアナリスト・DXコンサル・ITコンサル・PMO・研究職（バイオ・化学・材料等）・建築設計・土木・機械設計・電気設計・製造技術・生産技術・品質管理等のIT以外の理工系技術職も含む`);
  lines.push(`- 「文系職」: コンサル（IT以外）・営業（システム営業・IT営業含む）・マーケ・経営企画・財務・人事・金融営業・その他ビジネス職`);
  ...
  lines.push(`- 例外: 弁護士（法律事務所勤務）・公認会計士・税理士（監査法人・会計事務所勤務）は業態上、上場企業勤務がないのが通常のため社格要件を問わない。ただし年収基準は適用する`);
  ...
  if (isBizreach) {
    ...
    lines.push(`1. 絶対NG条件（アクセンチュア・ベイカレント・秘書・保守運用のみ・46歳以上IT理系技術職・46歳以上文系で財務経理歴なし・医師・教授等のアカデミア）に該当する → 即NG`);
    ...
    lines.push(`4. 文系職で大手・上場・外資勤務経験が一切ない（弁護士・公認会計士・税理士は除く） → NG`);
    lines.push(`5. 上記1〜4のいずれにも該当しない場合 → 必ずOKとすること`);
    lines.push(`6. 年収・在籍期間が不明で判断できない場合のみ「要確認」`);
    lines.push(`※弁護士・公認会計士・税理士は医師・教授とは異なり即NG対象外。年収基準のみ適用`);
  }
