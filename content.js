...
function buildCriteriaText(criteria, platform) {
  ...
  // 常時適用される標準基準
  lines.push(`【絶対NG】`);
  lines.push(`- 職歴にアクセンチュア（Accenture）が含まれる場合は即NG`);
  lines.push(`- 職歴にベイカレント（BayCurrent）が含まれる場合は即NG（年齢・職種問わず例外なし）`);
  lines.push(`- 職歴に野村総合研究所（NRI）が含まれる場合は即NG`);
  lines.push(`- 職歴にBIG4（デロイト トーマツ・PwC・EY・KPMG、およびそれらのコンサルティング・監査法人・グループ会社を含む）が含まれる場合は即NG`);
