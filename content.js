...
function buildCriteriaText(criteria, platform) {
  ...
  if (isBizreach) {
    ...
    lines.push(`- 学歴・社格は厳しく見ない。技術力・スキルセットを重視するため、学歴・勤務先の規模でNGにしない`);
    lines.push(`- ただし明らかに問題のある場合（Fラン私大かつ無名中小企業のみ等）は参考程度に考慮してよい`);
    ...
    lines.push(`3. 文系職で社格NG（中小・無名企業のみ）→ NG`);
    lines.push(`4. 文系職で学歴NG（①〜⑤のいずれにも非該当、かつ学歴が明記されている場合）→ NG`);
    lines.push(`5. 上記1〜4のいずれにも該当しない → OK`);
    lines.push(`※「要確認」は絶対に使用しないこと。必ずOKかNGを返すこと`);
    lines.push(`※ITエンジニア系は学歴・社格でNGにしない（ステップ3・4をスキップ）`);
  } else {
