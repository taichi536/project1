...
function buildCriteriaText(criteria, platform) {
  ...
  if (isBizreach) {
    ...
    lines.push(`\n▼年収`);
    lines.push(`- 1500万円以上：最優先OK候補`);
    lines.push(`- 1000〜1499万円：OK候補（学歴・社格・経験社数で絞り込む）`);
    lines.push(`- 1000万円未満（明記されている場合）：即NG`);
    lines.push(`- 年収不明・非公開：他の条件で判断（年収不明のみを理由にNGにしない）`);
    ...
    lines.push(`\n▼職種別の学歴・社格基準`);
    lines.push(`【ITエンジニア系（ソフトウェアエンジニア・SE・インフラ・クラウド・データエンジニア・データサイエンティスト・DXコンサル・ITコンサル・PMO・研究職・機械設計・電気設計・製造技術等の技術職）の場合】`);
    lines.push(`- 学歴：問わない（高卒・専門学校卒・海外大卒すべてOK）`);
    lines.push(`- 社格：問わない（中小・スタートアップでもOK）`);

    lines.push(`【文系職（コンサル・営業・マーケ・経営企画・事業企画・財務・人事・金融・その他ビジネス職）の場合】`);
    lines.push(`- 社格：大手企業・外資系・有名上場企業での勤務経験が必要。中小・無名企業のみはNG`);
    lines.push(`- 学歴：以下のいずれかに該当すること（いずれにも該当しない場合はNG）`);
    lines.push(`  ① 早慶上智（早稲田・慶應・上智）`);
    lines.push(`  ② MARCH（明治・青山学院・立教・中央・法政）`);
    ...
    lines.push(`  ※学歴が不明・記載なしの場合は学歴でNGにしない。年収・社格・経験社数で判断すること`);

    lines.push(`\n▼Bizreach判定方針（迷う場合は必ずNG）`);
    lines.push(`以下の順番で判定すること：`);
    lines.push(`1. 絶対NG条件に該当 → 即NG`);
    lines.push(`2. 年収1000万円未満（明記されている場合）→ NG`);
    lines.push(`3. 経験社数が年齢別上限を超える → NG`);
    lines.push(`4. 文系職で社格NG（中小・無名企業のみ）→ NG`);
    lines.push(`5. 文系職で学歴NG（①〜⑤のいずれにも非該当、かつ学歴が明記されている場合）→ NG`);
    lines.push(`6. 上記1〜5のいずれにも該当しない → OK`);
    lines.push(`※「要確認」は絶対に使用しないこと。年収・学歴・社格が不明でも必ずOKかNGを返すこと`);
    lines.push(`※ITエンジニア系は学歴・社格でNGにしない（ステップ4・5をスキップ）`);
  } else {
